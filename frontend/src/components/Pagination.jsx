...
function Pagination({ currentPage, pagesCount, setCurrentPage = () => { } }) {
  if (pagesCount && pagesCount < 2 || pagesCount === 0) return null
  ...
  return (
    <nav className='my-2 mx-auto'>
      <ul className="inline-flex -space-x-px text-sm">
        ...
        <>{numberButtons}</>
        <li>
          <button type='button' disabled={currentPage === pagesCount}
            onClick={() => { setCurrentPage(currentPage + 1) }}
  ...
  function createNumberButton(number, key) {
    return (
      <li key={key ? key : number}>
        <button type='button' disabled={number === currentPage || number === '...'}
          onClick={() => { if (number !== '...') setCurrentPage(number) }}
